...
PATCHES=( "${FILESDIR}"/${PN}-2.6.3-csu.patch 
	"${FILESDIR}"/${PN}-2.6.3-libsdp.patch )

src_unpack() {
	dragonfly_src_unpack

	sed -i.bak -e 's/iconv.h//' "${WORKDIR}"/${P}/include/Makefile || die
	sed -i.bak -e '/^MAN/d;/^MLINK/d' "${S}"/libc/iconv/Makefile.inc || die
...
# used.' \
	...
	sed -i.bank -e 's/-lprop/-L..\/libprop\/ -lprop/' ${S}/libdevattr/Makefile || die
}
...
src_install() {
	...
	gen_usr_ldscript libalias.so libatm.so libbind9.so libbluetooth.so \
		...
		libdevattr.so libldns.so libprop.so \
