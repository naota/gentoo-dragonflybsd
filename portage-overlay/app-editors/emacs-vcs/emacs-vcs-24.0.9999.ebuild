...
SLOT="24"
KEYWORDS="~alpha ~amd64 ~arm ~hppa ~ia64 ~ppc ~ppc64 ~s390 ~sh ~sparc ~x86 ~x86-fbsd ~x86-dfbsd"
...
src_prepare() {
	...
	cp "${FILESDIR}"/dragonfly.h "${S}"/src/s || die
	epatch "${FILESDIR}"/${P}-configure.in.patch

	sed -i -e "s:\$(CRT_DIR)/crtbegin.o:$(`tc-getCC` -print-file-name=crtbegin.o):g" \
	 	-e "s:\$(CRT_DIR)/crtend.o:$(`tc-getCC` -print-file-name=crtend.o):g" configure.in
